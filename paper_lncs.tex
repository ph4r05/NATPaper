...
\urldef{\mailsa}\path|xklinec@fi.muni.cz|
\urldef{\mailsb}\path|matyas@fi.muni.cz|
...
Network Address Translators (abbreviated as NATs) has been increasing in popularity in recent years, 
...
From an attacker perspective, relay servers can be seen as potential enemies trying to sniff and log all 
...
however does not work with symmetric NATs. ICE mainly uses STUN~\citep{rfc5389} and TURN\ignore{~\citep{rfc5766}} for its operation.

% \section{Related work}
Some interesting algorithms for traversing symmetric NAT were proposed by Y.~Takeda in \citep{takeda}, the case where both clients are behind symmetric NAT
...
Wang~\emph{et.~al.} \citep{Wang:2006:RSN:1156422.1156550} propose a NAT traversal algorithm for scenario where two hosts are
behind symmetric NATs, based on a changing of a source port, while in our approach we want to explore possibilities with constant source port.

In our work we study the situation where two parties -- A,B -- try to establish a direct connection, both of them are behind 
a symmetric NAT (possibly cascade of such NATs) with port sensitive, predictable allocation function. Other scenarios with
...
% \end{align*}%
 is a Socket. $\mathbb{IP}$ is set of all possible IP addresses and $\mathbb{PORT}$ 
